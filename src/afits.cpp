...
void repair_solution(const instance_data &ins, solution_data &fg_sol, solution_data &sd)
{
	...
	fg_sol.cost = calculate_fitness(ins, fg_sol.sol, fg_sol.weight);
...
void adaptive_fea_inf_tabu_search(char file[], instance_data &ins, solution_data &sd, solution_data &sd_best, int *tabu_list, int tabu_tenure, int tt_base, int ts_depth, clock_t start_time, double &run_time, double time_limit, double incre, int lambda)
{
	...
	//while (non_improve < ts_depth)
	{
		...
		//if (iter % 5000 == 0)
		//	cout << file << " ,iter=" << iter << ", sd_best.cost =" << sd_best.cost << ", time = " << elasped_time << endl;
	}
	...
	else
	{
		...
		fgsol_local_best.verify_solution(ins);
