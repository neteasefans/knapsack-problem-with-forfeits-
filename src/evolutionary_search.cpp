...
void initial_population(instance_data &ins, solution_data *pop, int pop_num, int ini_num, solution_data &sd_best, int *tabu_list, int tabu_tenure, int tt_base, int ts_depth,
	clock_t start_time, double &run_time, double time_limit, double incre, int lambda)
{
	...
	while (real_num < pop_num && index < ini_num)
	{
		...
		adaptive_fea_inf_tabu_search(ins, pop[real_num], sd_best, tabu_list, tabu_tenure, tt_base, ts_depth, start_time, run_time, time_limit, incre, lambda);
...
void output_stat_sol(char out_file[], char sol_file[], char in_file[], const instance_data &ins, const solution_data &sd)
{
	FILE *fp;
	char buff[MAXCAN];
	int select_num = 0;
	...
	for (int i = 0; i < ins.num_items; i++)
		if (sd.sol[i])
			select_num++;
	fprintf(fp, "%s cost:%d  cap:%d  weight:%d  items:%d  sels:%d \n", in_file, sd.cost, ins.capacity, sd.weight, ins.num_items, select_num);
	fclose(fp);
...
void evolutionary_search(char file_in[], char file_out[], char sol_out[], instance_data &ins, int tabu_tenure, int tt_base, int ts_depth, double time_limit, double incre, int lambda, int pop_num, int ini_num, double mut_str)
{
	...
	initial_population(ins, pop, pop_num, ini_num, sd_best, tabu_list, tabu_tenure, tt_base, ts_depth, start_time, run_time, time_limit, incre, lambda);
	//for (int i = 0; i < pop_num; i++)
	...
	while (elasped_time < time_limit)
	{
		...
		adaptive_fea_inf_tabu_search(ins, sd, sd_best, tabu_list, tabu_tenure, tt_base, ts_depth, start_time, run_time, time_limit, incre, lambda);
		//update_population(ins, sd, pop, pop_num);
		update_population_distance_quality(ins, sd, pop, pop_num);
		genes++;
		sd_best.verify_solution(ins);
		similarity = calculate_similarity_pop(ins, pop, pop_num);
	...
	if (verify)
	{
		...
		output_stat_sol(file_out, sol_out, file_in, ins, sd_best);
	}
	else
